...
test('index', async () => {
  ...
  const documentId = indexer.addDocument(input);
  indexer.flush();

  const document = storage.loadDocument(documentId);
  expect(document.text).toEqual(input);
  const index = storage.loadIndex('全文');
  ...
  const otherDocumentId = indexer.addDocument(otherInput);
  indexer.flush();
  const sameIndex = storage.loadIndex('全文');
